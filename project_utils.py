...
def draw_labels_and_boxes(img, boxes,    confidences, classids, idxs, colors, labels):
    ...
    if len(idxs) > 0:
        for i in idxs.flatten():
            ...
            text = "{}: {:4}".format(labels[classids[i]], str(round(confidences[i])) + "%")
            cv.putText(img, text, (x, y - 5), cv.FONT_HERSHEY_SIMPLEX, 0.75, color, 2)
...
def infer_image(net, layer_names, height, width, img, colors, labels, FLAGS,
    ...
    if infer:
        ...
        # Syntax: cv2.dnn.blobFromImage(image, scalefactor=1.0, size, mean, swapRB=True)
