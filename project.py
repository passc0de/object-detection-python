...
import time
import os
from project_utils import infer_image, show_image
...
if __name__ == '__main__':
    ...
    parser.add_argument('-m', '--model-path',
                        type=str,
                        default='./project-coco/',
                        help='The directory where the model weights and \
			  configuration files are.')

    parser.add_argument('-w', '--weights',
                        type=str,
                        default='./project-coco/yolov3.weights',
                        help='Path to the file which contains the weights \
			 	for YOLOv3.')

    parser.add_argument('-cfg', '--config',
                        type=str,
                        default='./project-coco/project.cfg',
    ...
    parser.add_argument('-l', '--labels',
                        type=str,
                        default='./project-coco/coco-labels',
    ...
    if FLAGS.image_path is None and FLAGS.video_path is None:
        ...
        print('               Capturing Webcam Footage              ')
       ...
       # fix_res()
        while True:
            ...
            if count == 0:
                ...
                frame, boxes, confidences, classids, idxs = infer_image(net, layer_names, height, width, frame, colors,
                                                                        labels, FLAGS, boxes, confidences, classids,
                                                                        idxs, infer=False)
                count = (count + 1) % 6

            cv.imshow('Object Detection: Project - WebCam', frame)
